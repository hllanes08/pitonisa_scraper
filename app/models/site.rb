...
class Site < ActiveRecord::Base
	...
	def search_and_delete(key)
		searches = SearchTag.where(tag:key, search_tag_date: Time.now)#"search_date betweeen ? and ? ", key, Time.now.beginning_of_day, Time.now.end_of_day)
		searches.each do |search|
			SearchPopularize.where(search_tag_id:search.id).delete_all
			search.delete
		end	
	end
